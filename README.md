...
平均値はすべての $N_i$ に対し、$\mu$ で固定する。$N_i$ の累積分布関数を $\Phi_i( x )$ とする。
また、接続区間 $Q_i = ( b_i, a_{i + 1} )$ について、その両端の確率に対する
X座標を $\beta_i, \alpha_{i+1}$ とする ($\Phi_i( \beta_i ) = b_i, \Phi_{i + 1}( alpha_{i+1} ) = a_{i + 1}$ )。
