...
| type1.type = 3 | $\Psi_i( x ) = \Phi_i( x ) - \dfrac{1}{ \sqrt{2} } \Phi^\ast_i( x ) + \dfrac{1}{ \sqrt{2} } \Phi^\ast_{i+1}( x )$ <br> $g_i( x ) = ( 1 - \dfrac{ f_i( x ) }{ f_i( \mu ) } ) f_i( x ) + \dfrac{ f_{i+1}( x ) }{ f_{i+1}( \mu ) } f_{i+1}( x )$ <br> (ただし $i \leq 2$) | 経路が3～4点<br> ( $x = \mu$ の1点および、 $x \neq -\infty, \mu, \infty$ の2～3点) | $[0, 0]$, $[0.5, 0.5]$, $[1, 1]$, $[0, 0.5]$, $[0.5, 1]$, $[0, 1]$ のいずれか | $x \neq \mu$ で連続 | 1.1.9902 (未テスト) |
...
| type1.type = 3 <br> continuous = TRUE | 無効 (type1.type = 3 のデフォルトと同じ) | - | - | - | 1.1.9902 (未テスト) |
