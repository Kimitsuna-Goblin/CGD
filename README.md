...
| :--------: | :--------------------------------------- | :------: | :----: | :----------------: | :-----: |
| type1.type = 1, <br> continuous = TRUE (or symmetric = TRUE) <br> (2つの正規分布の平均) | $\Psi( x ) = \dfrac{1}{2} ( \Phi_1( x ) + \Phi_2( x ) )$ <br> $g( x ) = \dfrac{1}{2} ( f_1( x ) + f_2( x ) )$ <br> where $\mu = \mu_1 = \mu_2$ | $( \mu, 0.5 )$ を含む3点 | $[0, 0]$, $[1, 1]$ の2点 | 連続<br>( $C^\infty$ 級) | 1.2.0 |
