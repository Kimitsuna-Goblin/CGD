...
なお、正規分布 $N( \mu, \sigma^2 )$ の累積分布関数 $\Phi_{N( \mu, \sigma^2 )}$ について、
X座標が平均値 $\mu$ から標準偏差 $\sigma$ の $n$ 倍離れた位置の確率 $\Phi_{N( \mu, \sigma^2 )}( n \sigma + \mu )$ の値は
$\sigma, \mu$ の値によらず、$n$ の値が決まれば一意に定まることから、
