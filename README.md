...
<br>
著作者のローカル環境では、CGD.stand-alone.R の元になった GCD.R は、この common.R を参照していましたが、
common.R の中にある GCD.R に必要だった関数は、すべて CGD.stand-alone.R にコピーしました。なので、このファイルは別に無くても動きます。
...
1. ある有限個の正規分布の集合に対して、その要素の正規分布に、拡大実数の範囲 [-∞, ∞] の部分閉区間をそれぞれ全て重複しないように割り当てる。
割り当てる閉区間は、有界であっても有界でなくてもよく、ただ1点の区間 [x, x] (x ∈ R) や、[-∞, -∞]、[∞, ∞] も許される。
ただし、割り当てる区間の条件として、区間同士が互いに重複しないことだけでなく、
割り当てた区間 [a, b] に対する累積分布の区間 [φ(a), φ(b)] (ただし、φ は割り当てられる分布の累積分布関数) も、すべて重複しないことを条件とする。
