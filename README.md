...
3. 隣り合う2つの閉区間 $I_i = [a_i, b_i], I_{i+1} = [a_{i+1}, b_{i+1}] ( 1 \leq i \leq n - 1 )$ に対し、
2つの累積分布関数 $\Phi_i$ と $\Phi_{i+1}$ を和や積などで組み合わせて、
$\Psi_i( b_i ) = \Phi_i( b_i ), \Psi_i( a_{i+1} ) = \Phi_{i+1}( a_{i+1} )$ を満たすような、
連続な単調増加関数 $\Psi_i$ を適当に定める。
ただし、$\Psi_i$ を定める際には、$\Phi_i$ と $\Phi_{i+1}$ はどのように組み合わせてもよいが、$\Phi_i$ と $\Phi_{i+1}$ 以外の累積分布関数は用いてはならない。
...
以上のように構成した累積分布関数を持つ確率分布を連結ガウス分布と呼ぶ。
