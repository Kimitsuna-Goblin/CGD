...
ただし、この式では、独立区間が $P_1 = [0, 0]$ または $P_n = [1, 1]$ のとき、係数の分母が $\pm\infty$ になるので、確率密度が計算できない。
...
このように定義すると、
    + 独立区間が $P_1 = [0, 0], P_2 = [1, 1]$ の場合
    + 独立区間が $P_1 = [0, 0], P_2 = [0.5, 0.5], P_3 = [1, 1]$ であって、$\Phi_1( x ) = \Phi_3( x )$ の場合
の2つの場合には、全区間 $(-\Inf, \Inf)$ で連続な確率密度関数を作ることができる。
