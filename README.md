...
添字のない $\mu$ はその分布の平均値を表し、添字のある $\mu_i$ は連結ガウス分布の構成要素の正規分布 $N_i( \mu_i, \sigma_i )$ の平均値を表す。原則として、 $\mu_i = \mu$ であるが、経路の点に $( \mu, 0.5 )$ を含まない場合および、 type1.type = 3 で経路の点が5点以上の場合は、一般に $\mu_i \neq \mu$ である。
