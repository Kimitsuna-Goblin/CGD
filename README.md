...
<p>
とにかく、これはそういった分布を生成する、\mathbb{R}言語のソースファイルです。


## 使い方 - How to use

<pre>
> source( "CGD.stand-alone.\mathbb{R}" )    # ソースファイルを読み込みます
...
2. 閉区間の集合 $I$ の要素数と等しい、n個の正規分布からなる集合 $N = \left\lbrace N_1, N_2, \cdots, N_n \right\rbrace$ を用意して、
...
$N_i, N_j$ の累積分布関数 $\Phi_i, \Phi_j$ の定義域が互いに重ならないこと。
すなわち、$\left\lbrace \Phi_i<sup>-1</sup>( p \in I_i ) \right\rbrace ∩ \left\lbrace \Phi_j<sup>-1</sup>( p \in I_j ) \right\rbrace = ∅$ であること。
...
特別な場合を除いて、一般に、上の2つの条件を満たす正規分布の集合 $N$ を構成することはできないが、
すべての閉区間 $I_i$ が高々1個の経路上の確率 $p_j$ しか含まない場合、特に、すべての閉区間 $I_i$ が1点のみからなる場合は、
2つの条件を満たす正規分布の集合 $N$ を必ず構成することができる (自明であろう)。

また、正規分布の平均値 $\mu$ を1つの値に固定して、ある点 x (\in \mathbb{R}) における確率 p (\in [0, 1]) が与えられたとき、
その累積分布関数が $\Phi( x ) = p, \Phi( \mu ) = 0.5$ を満たす正規分布は一意に定まるので、
すべての閉区間 $I_i$ が1点のみからなる場合は、
...
3. 隣接する2つの区間 $I_i = [a_i, b_i], I_{i+1} = [a_{i+1}, b_{i+1}] ( 1 \leq i \leq n - 1 )$ に対し、
$\Psi_i( b_i ) = \Phi_i( b_i ), \Psi_i( a_{i+1} ) = \Phi_{i+1}( a_{i+1} )$ を満たすような、
連続な単調増加関数 $\Psi_i$ を適当に定める。

以上のように関数 $\Phi_i, \Psi_i$ を定めたとき、
関数

$$
\Phi_{CGD}(x)=
\begin{cases}
\Phi_i(x) & \textrm{where } x \in [a_i, b_i] \\
\Psi_i(x) & \textrm{where } x \in (b_i, a_{i+1})
\end{cases}
$$

は累積分布関数としての要件を満たす。このような累積分布関数を持つ分布を連結ガウス分布と呼ぶ。

呼称について、閉区間 $I_i$ は、1つの正規分布がその累積分布関数を単独で独立的に負担していることから、これを「独立区間」と呼ぶ。
それに対して、関数 $\Psi_i ( 1 \leq i \leq n - 1 )$ が累積分布関数を負担している開区間は、
