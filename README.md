...
+ 累積分布関数の経路が与えられたときは、原則として、経路上の点は、1点のみからなる区間とする。ただし、以下の2つの場合を例外とする。

    + 区間 [0, 1] の両端である点 0 および点 1 は、1点のみからなる区間とはせずに、少なくとも、次の経路の点を含む区間とする。

    + 2つの連続する経路の点が、同じ正規分布の累積分布関数上の点となる場合 (ただし、正規分布の平均値は、経路の $p = 0.5$ の点に対応するX座標の値とする) は、
      それら2つの点を1つの区間に含める。ただし、いずれかの経路の点を区間の始点とし、他のいずれかの経路の点を区間の終点とする。

正規分布は以下のように構成する。

+ すべての正規分布の平均値は、経路で $p = 0.5$ の点として与えられた点のX座標の値とする。

+ 独立区間を昇順に並べたときの最初の独立区間 $[0, b_1]$ については、累積分布関数が区間の終点 $b_1$ を通る正規分布、
それ以外の独立区間については、累積分布関数が区間 $[a_i, b_i]$ の始点 $a_i$ を通る正規分布を採用する。
