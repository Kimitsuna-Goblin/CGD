...
1. まず、一般に確率が取りうる値の全体 $[0, 1]$ の部分閉区間の集合 $P$ を、その要素の区間同士が互いに重なり合うことがないように用意する。
ただし、区間のどれか1つに必ず 0 が含まれ、どれか1つに必ず 1 が含まれるようにする。
...
その要素 $P_i$ は、1点のみからなる区間 $[a_i, a_i] ( 0 \leq a_i \leq 1 )$ でもよい。
また、それぞれの閉区間 $P_i$ は昇順に並んでいることが望ましい。
すなわち、$1 \leq i \leq n - 1$ に対して、$b_i ＜ a_{i+1}$ となるように構成しておくことが望ましい
 (以下、$P$ はそのように構成されているものとする)。

ここで、累積分布関数の経路 (あるいはクォンタイル。以下、経路と言う) $W = \left\lbrace ( x_j, p_j ) \in ( \mathbb{R}, [0, 1] ) \mid 1 \leq j \leq m \right\rbrace$ が与えられているときは、
...
同様に、$P_{i+1} = [a_{i+1}, b_{i+1}]$ に対する $\Phi_{i+1}( x )$ の定義域を $[\alpha_{i+1}, \beta_{i+1}]$ と書く
 (つまり、$\alpha_i = \Phi_i^{-1}( a_i ), \beta_i = \Phi_i^{-1}( b_i )$ とする。同様に、$\alpha_{i+1} = \Phi_{i+1}^{-1}( a_{i+1} ), \beta_{i+1} = \Phi_{i+1}^{-1}( b_{i+1} )$ とする)。

3. 隣り合う2つの閉区間 $P_i = [a_i, b_i], P_{i+1} = [a_{i+1}, b_{i+1}] \quad ( 1 \leq i \leq n - 1 )$ に対し、
...
以下、2つの独立区間 $P_i = [a_i, b_i], P_{i + 1} = [a_{i + 1}, b_{i + 1}]$ が互いに隣り合っており、
それらの独立区間を負担する正規分布の平均値を $\mu$、標準偏差を $\sigma_i, \sigma_{i + 1}$ とし、累積分布関数を $\Phi_i( x ), \Phi_{i + 1}( x )$ とする。
また、それら2つの区間の間の接続区間 $( b_i, a_{i + 1} )$ について、その両端の確率に対する
X座標を $x_{1}, x_{2}$ とする ($\Phi_i( x_1 ) = b_i, \Phi_{i + 1}( x_2 ) = a_{i + 1}$ )。
