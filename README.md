...
| type1.type = 3 | $\Psi_i( x ) = \int_{-\infty}^x \lbrace \( 1 - \dfrac{ f_i( t ) }{ f_i( \mu ) } ) f_i( t ) + \dfrac{ f_{i+1}( t ) }{ f_{i+1}( \mu ) } f_{i+1}( t ) \rbrace dt$ <br> $g_i( x ) = ( 1 - \dfrac{ f_i( x ) }{ f_i( \mu ) } ) f_i( x ) + \dfrac{ f_{i+1}( x ) }{ f_{i+1}( \mu ) } f_{i+1}( x )$ <br> (ただし $i \leq 2$ ) | 経路が5点以下<br> ( $x = \mu$ の1点および、 $-\infty < x < \mu$ と $\mu < x < \infty$ の各2点以下) | $[0, 0]$, $[0.5, 0.5]$, $[1, 1]$, $[0, 0.5]$, $[0.5, 1]$, $[0, 1]$ のいずれか | $x \neq \mu$ で連続 | 未実装 |
