...
連結ガウス分布の確率密度関数 $f( x )$ は $x = \beta_i, \alpha_{i+1}$ の2点で不連続になる。

それから、この式では、独立区間が $P_1 = [0, 0]$ または $P_n = [1, 1]$ のとき、係数の分母が $\pm\infty$ になるので、確率密度が計算できない。
このことは現バージョン (1.0.0) の欠点である。
...
そのため、接続関数 $\Psi_i( x )$ は、値が $Q_i$ の範囲から出ないように注意して、以下のように構成する。
...
接続関数 $\Psi_i( x )$ は、値が $Q_i$ の範囲から出ないように、以下のように構成する。
