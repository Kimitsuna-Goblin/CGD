...
| オプション  | 接続関数 $\Psi_i(x)$ ・確率密度関数 $g_i(x)$ | 適用可能な経路の条件 | 独立区間 |確率密度関数の連続性 | 実装ver |
| :--------: | :--------------------------------------- | :------: | :----: | :----------------: | :-----: |
...
| type1.type = 3 | $\Psi_i( x ) = \int_{-\infty}^x \lbrace ( 1 - \dfrac{ f_i( t ) }{ f_i( \mu ) } ) f_i( t ) + \dfrac{ f_{i+1}( t ) }{ f_{i+1}( \mu ) } f_{i+1}( t ) \rbrace dt$ <br> $\quad \quad \  = \Phi_i( x ) - \dfrac{ 1 + \mathrm{ erf }( \dfrac{ x - \mu }{ \sigma_i } ) }{ 2 \sqrt{ 2 \sigma_i^2 } } + \dfrac{ 1 + \mathrm{ erf }( \dfrac{ x - \mu }{ \sigma_{i+1} } ) }{ 2 \sqrt{ 2 \sigma_{i+1}^2 } }$ <br> $g_i( x ) = ( 1 - \dfrac{ f_i( x ) }{ f_i( \mu ) } ) f_i( x ) + \dfrac{ f_{i+1}( x ) }{ f_{i+1}( \mu ) } f_{i+1}( x )$ <br> (ただし $i \leq 2$ ) | 経路が5点以下<br> ( $x = \mu$ の1点および、 $-\infty < x < \mu$ と $\mu < x < \infty$ の各2点以下) | $[0, 0]$, $[0.5, 0.5]$, $[1, 1]$, $[0, 0.5]$, $[0.5, 1]$, $[0, 1]$ のいずれか | $x \neq \mu$ で連続 | 未実装 |
...
| type1.type = 3 <br> symmetric = TRUE |  $\Psi( x ) = \int_{-\infty}^x \lbrace ( 1 - \dfrac{ f_1( t ) }{ f_1( \mu ) } ) f_1( t ) + \dfrac{ f_2( t ) }{ f_2( \mu ) } f_2( t ) \rbrace dt$ <br> $\quad \quad \  = \Phi_1( x ) - \dfrac{ 1 + \mathrm{ erf }( \dfrac{ x - \mu }{ \sigma_1 } ) }{ 2 \sqrt{ 2 \sigma_1^2 } } + \dfrac{ 1 + \mathrm{ erf }( \dfrac{ x - \mu }{ \sigma_2 } ) }{ 2 \sqrt{ 2 \sigma_2^2 } }$ <br> $g( x ) = ( 1 - \dfrac{ f_1( x ) }{ f_1( \mu ) } ) f_1( x ) + \dfrac{ f_2( x ) }{ f_2( \mu ) } f_2( x )$ | 経路が3点<br> ( $x = \mu$ の1点と $x \neq -\infty, \mu, \infty$ の2点) | $[0, 0]$, $[0.5, 0.5]$, $[1, 1]$ の3点 | 連続<br>( $C^\infty$ 級) | 未実装 |
