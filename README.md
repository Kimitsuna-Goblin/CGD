...
| type1.type = 3 | $\Psi_i( x ) = \Phi_i( x ) - \dfrac{ \mathrm{erf}( \dfrac{ x - \mu }{ \sigma_i } ) }{ 4 f_i( \mu ) \sqrt{ \pi \sigma_i^2 } } + \dfrac{ \mathrm{erf}( \dfrac{ x - \mu }{ \sigma_{i+1} } ) }{ 4 f_{i+1}( \mu ) \sqrt{ \pi \sigma_{i+1}^2 } }$ <br> $g_i( x ) = ( 1 - \dfrac{ f_i( x ) }{ f_i( \mu ) } ) f_i( x ) + \dfrac{ f_{i+1}( x )^2 }{ f_{i+1}( \mu ) }$ <br> (ただし $i \leq 2$ ) | 経路が5点以下<br> ( $x = \mu$ の1点および、 $-\infty < x < \mu$ と $\mu < x < \infty$ の各2点以下) | $[0, 0]$, $[0.5, 0.5]$, $[1, 1]$, $[0, 0.5]$, $[0.5, 1]$, $[0, 1]$ のいずれか | $x = \mu$ 以外で連続 |
...
| type1.type = 1 <br> symmetric = TRUE | type1.type = 1, continuous = TRUE と同じ | - | - |
