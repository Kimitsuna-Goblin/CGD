...
このとき、n = m として、1つの閉区間 I<sub>i</sub> ( 1 ≦ i ≦ n ) に確率 p<sub>i</sub> が1つだけ含まれるように I を構成すれば、後の工程も含めて容易になるが、
もし、後の工程が可能であれば、1つの閉区間 I<sub>i</sub> に複数の確率 p<sub>j</sub>, p<sub>j+1</sub>, p<sub>j+2</sub>, ... が含まれるように構成してもよい。

ある有限個の正規分布からなる集合 S<sub>N</sub> = { N<sub>1</sub>, N<sub>2</sub>, ..., N<sub>n</sub> } を用意して、
その要素の正規分布 N<sub>i</sub> ( 1 ≦ i ≦ n ) に、
区間 [0, 1] の部分閉区間 I<sub>1</sub>, I<sub>2</sub>, ..., I<sub>n</sub> ( ⊂ [0, 1] ) をそれぞれ割り当てる。各部分閉区間は互いに重複しないようにする。
つまり、i ≠ j ⇒ I<sub>i</sub> ∩ I<sub>j</sub> = ∅ となるようにする。
