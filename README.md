...
1. ある有限個の正規分布の集合に対して、その要素の正規分布に、実数の区間 [0, 1] の部分閉区間を異なる2つの区間が互いに重複しないように、それぞれ割り当てる。
割り当てる区間は、有界であっても有界でなくてもよく、1点からなる区間 [x, x] (x ∈ [0, 1]) も許される。
ただし、区間を割り当てる条件として、割り当てた区間同士が互いに重複しないことだけでなく、
割り当てた区間 [a, b] に対する累積分布の根の区間 [φ<sup>-1</sup>(a), φ<sup>-1</sup>(b)]
(ただし、φ は割り当てられる分布の累積分布関数。φ<sup>-1</sup> はその逆関数) も、すべて互いに重複しないことを条件とする。
これらの、1つの正規分布に対して1つずつ独立的に割り当てられた閉区間を「独立区間」と呼ぶ。
そして、割り当てられた区間において、連結ガウス分布の累積分布関数は、区間その正規分布の累積分布関数に一致するように定める。
