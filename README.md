...
+ 昇順に並べた独立区間の最初の区間 $[0, b_1]$ については、累積分布関数が独立区間の終点 $b_1$ とそのときの確率 $p_{b_1}$ を通る正規分布、
それ以外の区間については、累積分布関数が独立区間 $[a_i, b_i]$ の始点 $a_i$ とそのときの確率 $p_{a_i}$ を通る正規分布を採用する。

ところで、正規分布 $N( \mu, \sigma^2 )$ の累積分布関数 $\Phi_{\mu, \sigma}( x )$ について、
X座標が平均値 $\mu$ から標準偏差 $\sigma$ の $n$ 倍離れた位置の確率 $\Phi_{\mu, \sigma}( \mu + n \sigma )$ の値は
$\mu, \sigma$ の値に関係なく、$n$ の値が決まれば一意に定まる (ちなみに、その値は誤差関数 $\mathrm{ erf }( x )$ を使って書くと、$( 1 + \mathrm{ erf }( n / \sqrt{2\pi} ) ) / 2$ である)。
...
$$
\sigma = \frac{ q - \mu }{ \Phi^{-1}( p ) }
$$

で求められる。
この式を使えば、平均値を固定したとき、累積分布関数がある1点を通るような正規分布の標準偏差が一意に得られるので、
