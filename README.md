...
| type1.type = 3 | $\Psi_i( x ) = \Phi_i( x ) - \dfrac{ \mathrm{erf}( \dfrac{ x - \mu }{ \sigma_i } ) }{ 4 f_i( \mu ) \sqrt{ \pi \sigma_i^2 } } + \dfrac{ \mathrm{erf}( \dfrac{ x - \mu }{ \sigma_{i+1} } ) }{ 4 f_{i+1}( \mu ) \sqrt{ \pi \sigma_{i+1}^2 } }$ <br> $g_i( x ) = ( 1 - \dfrac{ f_i( x ) }{ f_i( \mu ) } ) f_i( x ) + \dfrac{ f_{i+1}( x )^2 }{ f_{i+1}( \mu ) }$ <br> (ただし $i \leq 2$ ) | 経路が5点以下<br> ( $x = \mu$ の1点および、 $-\infty < x < \mu$ と $\mu < x < \infty$ の各2点以下) | $[0, 0]$, $[0.5, 0.5]$, $[1, 1]$, $[0, 0.5]$, $[0.5, 1]$, $[0, 1]$ のいずれか | $x \neq \mu$ で連続 |
...
| type1.type = 3 <br> continuous = TRUE | 無効 (type1.type = 3 のデフォルトと同じ) | - | - |
...
| type1.type = 3 <br> symmetric = TRUE | $\Psi( x ) = \Phi_1( x ) - \dfrac{ \mathrm{erf}( \dfrac{ x - \mu }{ \sigma_1 } ) }{ 4 f_1( \mu ) \sqrt{ \pi \sigma_1^2 } } + \dfrac{ \mathrm{erf}( \dfrac{ x - \mu }{ \sigma_2 } ) }{ 4 f_2( \mu ) \sqrt{ \pi \sigma_2^2 } }$ <br> $g( x ) = ( 1 - \dfrac{ f_1( x ) }{ f_1( \mu ) } ) f_1( x ) + \dfrac{ f_2( x )^2 }{ f_2( \mu ) }$ | 経路が3点<br> ( $x = \mu$ の1点と $x \neq -\infty, \mu, \infty$ の2点) | $[0, 0]$, $[0.5, 0.5]$, $[1, 1]$ の3点<br>(ただし、 $[0, 0]$ と $[1, 1]$ は $N_1$ の独立区間) | 連続<br>( $C^\infty$ 級) |
