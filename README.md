...
この場合は、接続関数 $\Psi_i( x )$ の値が $Q_i$ の範囲から出ないように注意して、以下のように $\Psi_i( x )$ を構成する。
...
$\Phi_i( x )$ は $\mu < x < \alpha_{i+1}$ の範囲の後半部分で $\Phi_i( x ) > a_{i+1}$ となり、$Q_i$ の外に出ることがあるので、
接続関数 $\Psi_i( x )$ の値が $Q_i$ の範囲から出ないように注意して、以下のように $\Psi_i( x )$ を構成する。
