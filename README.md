...
この際、仮に、$n$ を $m$ と等しく取り、それぞれ1つの閉区間 $P_i ( 1 \leq i \leq n = m )$ に確率 $p_i$ が1つだけ含まれるように $P$ を構成したならば、
後の工程が確実に実行できる。
ただし、後の工程が実行可能であれば、1つの閉区間 $P_i$ に複数の確率 $p_j, p_{j+1}, p_{j+2}, \cdots$ が含まれるように構成してよい。
