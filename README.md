...
これまでに述べた閉区間 $P_i$ と開区間 $Q_i$ の呼称について、
閉区間 $P_i =  [a_i, b_i]$ は、1つの正規分布の累積分布関数 $\Phi_i(x)$ が単独で、
連結ガウス分布の累積分布関数 $\Phi_{CGD}(x)$ を独立的に負担している区間であることから、以降、これを「独立区間」と呼ぶ。
それに対して、開区間 $Q_i = (b_i, a_{i+1})$ は、2つの独立区間を接続していることから、以降、これを「接続区間」と呼ぶ。
...
この場合は、累積分布関数 $\Phi_i( x ), \Phi_{i + 1}(x)$ は両方とも、
