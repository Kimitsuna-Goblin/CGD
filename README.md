...
| type1.type = 3 <br> symmetric = TRUE | $\Psi_i( x ) = \Phi_i( x ) - \dfrac{ \mathrm{erf}( \dfrac{ x - \mu }{ \sigma_i } ) }{ 4 f_i( \mu ) \sqrt{ \pi \sigma_i^2 } } + \dfrac{ \mathrm{erf}( \dfrac{ x - \mu }{ \sigma_{i+1} } ) }{ 4 f_{i+1}( \mu ) \sqrt{ \pi \sigma_{i+1}^2 } }$ <br> $g_i( x ) = ( 1 - \dfrac{ f_i( x ) }{ f_i( \mu ) } ) f_i( x ) + \dfrac{ f_{i+1}( x )^2 }{ f_{i+1}( \mu ) }$ <br> (ただし $i = 1, 2, \  \Phi_3 = \Phi_1, \  f_3 = f_1$ ) | 経路が3点<br> ( $x = \mu$ の1点と $x \neq -\infty, \mu, \infty$ の2点) | $[0, 0]$, $[0.5, 0.5]$, $[1, 1]$ の3点 | 連続<br>( $C^\infty$ 級) |
