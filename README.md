...
連結ガウス分布とは、
正規分布 (ガウス分布) の累積分布関数を区間で切断し、隙間を持たせてX軸方向に並べ、
それが連続になるように連結させた確率分布です。
...
すなわち、$1 \leq i \leq n - 1$ に対して、$b_i ＜ a_{i+1}$ となっていることが望ましい
 (以下、$P$ はこれらの条件を満たすものとする)。

ここで、予め、累積分布関数の経路 (あるいはクォンタイル。以下、経路と言う)
...
この際、$n = m$ として、それぞれ1つの閉区間 $P_i ( 1 \leq i \leq n )$ に確率 $p_i$ が1つだけ含まれるように $P$ を構成すれば、後の工程が確実に実行できる。
...
閉区間 $P_i =  [a_i, b_i]$ は、1つの正規分布の累積分布関数 $\Phi_i(x)$ が連結ガウス分布の $\Phi_{CGD}(x)$ を単独で独立的に負担している区間であることから、これを「独立区間」と呼ぶ。
...
+ 先頭の独立区間 $[0, b_1]$ に対しては、累積分布関数が終点 $b_1$ とそのときのX座標 $x_{b_1}$ を通る正規分布を採用し、
それ以外の独立区間 $[a_i, b_i]$ に対しては、累積分布関数が始点 $a_i$ とそのときのX座標 $x_{a_i}$ を通る正規分布を採用する。
