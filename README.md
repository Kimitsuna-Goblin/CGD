...
なお、表中の式で、 $\Phi_i( x ), \Phi_{i+1}( x )$ は正規分布 $N_i,N_{i+1}$ の累積分布関数、 $f_i( x ), f_{i+1}( x )$ はそれらの分布の確率密度関数、 $\bar \Phi_i( x ) = ( \Phi_i( x ) + \Phi_{i+1}( x ) ) / 2$ 、 $\Phi^\ast_i(x)$ は正規分布 $N( \mu, \dfrac{ \sigma_i }{ \sqrt2 } )$ の累積分布関数 である。
...
| type1.type = 3 | $\Psi_1( x ) = \Phi_1( x ) - \dfrac{1}{ \sqrt{2} } \Phi^\ast_1( x ) + \dfrac{1}{ \sqrt{2} } \Phi^\ast_2( x )$ <br> $\Psi_2( x ) = \Phi_3( x ) - \dfrac{1}{ \sqrt{2} } \Phi^\ast_3( x ) + \dfrac{1}{ \sqrt{2} } \Phi^\ast_2( x )$ <br> $g_1( x ) = ( 1 - \dfrac{ f_1( x ) }{ f_1( \mu ) } ) f_1( x ) + \dfrac{ f_2( x ) }{ f_2( \mu ) } f_2( x )$ <br> $g_2( x ) = ( 1 - \dfrac{ f_3( x ) }{ f_3( \mu ) } ) f_3( x ) + \dfrac{ f_2( x ) }{ f_2( \mu ) } f_2( x )$ | 経路が3～4点<br> ( $x = \mu$ の1点および、 $x \neq -\infty, \mu, \infty$ の2～3点) | $[0, 0]$, $[0.5, 0.5]$, $[1, 1]$, $[0, 0.5]$, $[0.5, 1]$, $[0, 1]$ のいずれか | 連続<br>( $C^\infty$ 級) | テスト中 |
...
| type1.type = 3 <br> continuous = TRUE | 無効 (type1.type = 3 のデフォルトと同じ) | - | - | - | テスト中 |
