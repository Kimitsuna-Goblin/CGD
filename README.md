...
ここで、ある平均値 $\mu$ の正規分布があって、その累積分布関数が、点 $x = q$ で確率 $p$ を取るときに、その分布の標準偏差 $\sigma$ を $\mu, q, p$ を使って表すことを考える。
...
なお、不連続な連結ガウス分布の場合、すべての $N_i$ の平均値は等しい値 (経路の点 $( x, 0.5 )$ の $x$ の値) に取る。そして、その値はそのまま連結ガウス分布の平均値になる。以下、その値を添字のない $\mu$ と書く (つまり、不連続な分布の場合、 $\mu_i = \mu$ である)。
...
表中の式で、 $\Phi_i( x ), \Phi_{i+1}( x )$ は正規分布 $N_i,N_{i+1}$ の累積分布関数である。
$f_i( x ), f_{i+1}( x )$ は同じく正規分布 $N_i,N_{i+1}$ の確率密度関数である。
添字のない $\mu$ は連結ガウス分布の平均値を表し、添字のある $\mu_i$ は構成要素の正規分布 $N_i = N( \mu_i, \sigma_i^2 )$ の平均値を表す。
