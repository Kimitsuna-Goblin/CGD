...
本ライブラリのバージョン 1.0.0 (または、バージョン 1.1.0 のデフォルト) では、
...
この欠点を解消するために、
バージョン 1.1.0 では、コンストラクタ new() の引数に type1.type = 2 を指定することで、
接続関数が
...
ただし、バージョン 1.1.0 では、そのような確率密度関数を作る処理は未実装である。
バージョン 1.1.0 では、そのような独立区間を set.waypoints() で指定しようとすると、警告になり、確率分布が構成されない。
